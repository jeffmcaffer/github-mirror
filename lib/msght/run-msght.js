...
appInsights.setup(process.env.APPINSIGHTS_INSTRUMENTATIONKEY).setAutoCollectExceptions(true).start();
const appInsightsClient = appInsights.getClient(process.env.APPINSIGHTS_INSTRUMENTATIONKEY);
...
function startProcess(key, org, repo, id, keyI, orgI, num_tried) {
    ...
    proc_active[id] = 1;
    var proc = spawn('ruby', ['-Ilib', __dirname + '/../../bin/ght-retrieve-repo',
                              '-c', __dirname + '/../../config.yaml', '-t',
                              key, '-l', 50000, org, repo],
                              {detached: true});
    process_map[proc.pid] = 0; //Mark that the process is running
    proc_pid[id] = proc.pid;
    proc_access[id] = 0; //Mark unknown access for this repository
...
function exitProcess(id, proc, pid, code, signal, repo, org) {
    ...
    if(signal == null && process_map[proc.pid] == 0 && proc_pid[id] == pid) {
            ...
            appInsightsClient.trackEvent("Repo Data Gathered", {SpawnID: id.toString(),
